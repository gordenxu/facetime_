...
from .types import Bbox, Frame, Kps, Mask, Matrix, Size, Template
...
def warp_face(temp_frame: Frame, kps: Kps, template: Template, size: Size) -> Tuple[Frame, Matrix]:
	...
	affine_matrix = cv2.estimateAffinePartial2D(
		kps, normed_template, method=cv2.RANSAC, ransacReprojThreshold=100
	)[0]
	crop_frame = cv2.warpAffine(
		temp_frame, affine_matrix, (size[1], size[1]), borderMode=cv2.BORDER_REPLICATE
	)
...
def paste_back(temp_frame: Frame, crop_frame: Frame, crop_mask: Mask, affine_matrix: Matrix) -> Frame:
	...
	inverse_crop_frame = cv2.warpAffine(
		crop_frame, inverse_matrix, temp_frame_size, borderMode=cv2.BORDER_REPLICATE
	)
	paste_frame = temp_frame.copy()
	for c in range(3):
		paste_frame[:, :, c] = (
			inverse_crop_mask * inverse_crop_frame[:, :, c]
			+ (1 - inverse_crop_mask) * temp_frame[:, :, c]
		)
	return paste_frame
...
def distance_to_bbox(points: numpy.ndarray[Any, Any], distance: numpy.ndarray[Any, Any]) -> numpy.ndarray[Any, Any]:
